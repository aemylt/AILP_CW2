...
create_actor_data([CurActor|ActorNames], Processed, Actors) :-
   wp(CurActor, WT),
   findall(L, wt_link(WT, L), Links),
   create_actor_data(ActorNames, [actor_data(CurActor, Links)|Processed], Actors).
filter_actors([], _, Filtered, Filtered).
filter_actors([Actor|Unfiltered], Link, Filtering, Filtered) :-
   Actor = actor_data(_,Links),
   (memberchk(Link, Links) -> filter_actors(Unfiltered, Link, [Actor|Filtering], Filtered)
   ; filter_actors(Unfiltered, Link, Filtering, Filtered)).
